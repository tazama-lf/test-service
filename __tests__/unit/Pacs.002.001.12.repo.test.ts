...
import { Pacs002Repo } from '../../src/repositories/raw_history/Pacs.002.001.12.repository';
...
describe('Pacs002Repo', () => {
  ...
  describe('list', () => {
    it('returns mapped data + total when rows exist', async () => {
      ...
      const res = await Pacs002Repo.list({
        limit: 10,
        offset: 0,
        sort: 'TxTp',
        order: 'DESC',
  ...
  describe('create', () => {
    it('inserts and returns document', async () => {
      ...
      const res = await Pacs002Repo.create(payload);
